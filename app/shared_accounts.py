...
def _remote_origin():
    receive = current_app.config.get("UMSHADO_SSO_RECEIVE_URL") or ""
...
def register():
    ...
    invite_token = request.form.get("invite_token", "")
    if account_type not in {"organizer", "vendor"} or invite_token:
    ...
    if db.session.scalar(select(User).where(User.email == email)):
        return _register_page("An Umcimby account with that email already exists. Please log in.", "error", 409)
    ...
    return redirect(url_for("vendors.setup_store" if account_type == "vendor" else "main.setup_event"))
