...
class EventPaymentHandler(PaymentHandler):
    ...
    def on_payment_completed(self, record, webhook):
        ...
        if row.kind == "owner_upgrade":
            row.event.plan_tier = "standard"
            row.event.upgraded_at = datetime.now(timezone.utc)
            ...
            if row.kind == "owner_pays_invite":
                ...
                self._ensure_member(invitation.event_id, row.user_id, invitation.role)
        db.session.commit()
    ...
    @staticmethod
    def _ensure_member(event_id, user_id, role="stakeholder"):
        ...
        if existing is None:
            db.session.add(EventMember(event_id=event_id, user_id=user_id, role=role))


def build_gateway():
    return MojaposPayments(MojaposConfig.from_env(), handler=EventPaymentHandler())
