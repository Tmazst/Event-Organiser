...
from .models import Invitation, Payment, EventMember
from .payment_gateway import complete_mock_payment
from .routes import current_event
...
def pricing():
    ...
    return render_template(
        "billing/pricing.html", event=event,
...
def upgrade():
    ...
    if event.plan_tier == "standard":
        flash("This event is already on the Standard plan.", "info")
...
def join_invitation(token):
    ...
    if expires_at < datetime.now(timezone.utc):
        flash("This invitation has expired. Ask the event owner for a new link.", "error")
        return redirect(url_for("billing.accept_invitation", token=token))
    if invitation.event.owner_id == current_user.id:
        flash("The event owner cannot join through their own invitation.", "error")
        return redirect(url_for("main.dashboard"))
    ...
    if invitation.payer == "owner":
        ...
        if existing is None:
            db.session.add(EventMember(
                event_id=invitation.event_id, user_id=current_user.id, role=invitation.role
            ))
        invitation.status = "accepted"
        invitation.accepted_by_user_id = current_user.id
        invitation.accepted_at = datetime.now(timezone.utc)
        db.session.commit()
        flash("You have joined the event project.", "success")
        return redirect(url_for("main.dashboard"))
